...
class Semester:
    ...
    def totalCredits(self):
        # YOUR CODE STARTS HERE
        totCreds = 0
        for courseObj in self.courses.values():
...
class Staff(Person):
    ...
    def setSupervisor(self, new_supervisor):
        # YOUR CODE STARTS HERE
        if isinstance(new_supervisor, Staff) and self != new_supervisor:
...
class Student(Person):
    ...
    def enrollCourse(self, cid, catalog):
        ...
        if not self.hold and self.active:
            ...
            if cid in catalog.courseOfferings:
                if cid not in self.semesters[semNum].courses:
                    self.semesters[semNum].addCourse(catalog.courseOfferings[cid])
                    self.account.chargeAccount(int(self.account.CREDIT_PRICE * catalog.courseOfferings[cid].credits))
                    return 'Course added successfully'
                else:
    ...
    def getLoan(self, amount):
        ...
        if self.active:
            ...
            if len(self.semesters) != 0:
                semNum = max(self.semesters.keys())
            if self.semesters[semNum].isFullTime:
                currLoan = Loan(amount)
