...
class Catalog:
    ...
    def _loadCatalog(self, file):
        ...
        # YOUR CODE STARTS HERE
        for line in course_info:                                                #make a list [cid, cname, credits] by removing \n and splitting by ',' 
            line = line.rstrip('\n')
            courseLst = line.split(',')
            self.addCourse(courseLst[0], courseLst[1], int(courseLst[2]))               
...
class Semester:
    ...
    def totalCredits(self):
        # YOUR CODE STARTS HERE
        totCreds = 0
        for courseObj in self.courses.values():                 #add up credits of the courses in the dict
            totCreds += courseObj.credits
        return totCreds
...
class Person:
    ...
    def __eq__(self, other):
        # YOUR CODE STARTS HERE
        if isinstance(other, Person) or issubclass(other, Person):
...
class Staff(Person):
    ...
    def id(self):
        ...
        initials = (firstLast[0][0] + firstLast[1][0]).lower()
    ...
    def setSupervisor(self, new_supervisor):
        # YOUR CODE STARTS HERE
        if isinstance(new_supervisor, Staff) and self != new_supervisor:                    #update self's supervisor if the new one is a staff and not self
    ...
    def applyHold(self, student):
        # YOUR CODE STARTS HERE
        if isinstance(student, Student) and self != student:
            student.hold = True
            return "Completed!"
        return None
...
class Student(Person):
    ...
    def registerSemester(self):
        # YOUR CODE STARTS HERE
        if not self.hold and self.active:                                  # get the new semester number which is 1 if no sem before or latest sem + 1 and add key:value to semesters dict and update the students year based on the semesters attended
            semNum = 1
            if len(self.semesters) != 0:
                semNum = max(self.semesters.keys()) + 1
            self.semesters[semNum] = Semester()
            if 1 <= len(self.semesters) <= 2:
                self.classCode = 'Freshman'
            elif 3 <= len(self.semesters) <= 4:
                self.classCode = 'Sophomore'
            elif 5 <= len(self.semesters) <= 6:
    ...
    def enrollCourse(self, cid, catalog):
        # YOUR CODE STARTS HERE
        if not self.hold and self.active:                                           # proceed if not on hold and active
            semNum = 1                                                              # get the latest semester number
            if len(self.semesters) != 0:
                semNum = max(self.semesters.keys())
            if cid in catalog.courseOfferings:                                      # check if college offers course
                if cid not in self.semesters[semNum].courses:                       # does student take it? if not add it, if so say already enrolled
                    self.semesters[semNum].addCourse(catalog.courseOfferings[cid])
                    self.account.chargeAccount(int(self.account.CREDIT_PRICE * catalog.courseOfferings[cid].credits))  # charge student account by num credits of course * the credit price
                    return 'Course added successfully'
                else:
    ...
    def dropCourse(self, cid):                                                      # very similar to enrollCourse but instead of adding remove if found and reduce tuition/account balance by half the cost of the course
        # YOUR CODE STARTS HERE
        if not self.hold and self.active:
            semNum = 1
            if len(self.semesters) != 0:
                semNum = max(self.semesters.keys())
            if cid in self.semesters[semNum].courses:
                temp = self.semesters[semNum].courses[cid]
                self.semesters[semNum].dropCourse(self.semesters[semNum].courses[cid])
                self.account.makePayment(self.account.CREDIT_PRICE * temp.credits * 0.5)
    ...
    def getLoan(self, amount):
        # YOUR CODE STARTS HERE
        if self.active:                                 #check if active if not return unsuccsessful operation
            semNum = 1                                  # get current semester number
            if len(self.semesters) != 0:
                semNum = max(self.semesters.keys())
            if self.semesters[semNum].isFullTime:       #only give loan if the student is full time
                currLoan = Loan(amount)                 # make loan object and add it to the student's account loan dictionary then make a payment to the account balance
                self.account.loans[currLoan.loan_id] = currLoan
                self.account.makePayment(currLoan.amount)
                return None
            else:
...
def run_tests():
    ...
    # doctest.run_docstring_examples(Loan, globals(), name='HW2',verbose=False)   
