...
class ContentItem:
    ...
    def __hash__(self): 
        ...
        sum = 0
        for i in self.header: # sum of the ascii values of the header
            sum += ord(i)
        return sum % 3
...
class CacheList:
    ...
    def put(self, content, evictionPolicy):
        # YOUR CODE STARTS HERE
        if content.size > self.maxSize: # content size is greater than cache size
            return 'Insertion not allowed'
        if content.cid in self: # content already in cache
            return f'Content {content.cid} already in cache, insertion not allowed'
        while self.remainingSpace < content.size and self.numItems > 0: # evict until there is enough space
    ...
    def clear(self):
        # Unlink all nodes in the doubly linked list
        current = self.head
        while current:
            next = current.next
            current.prev = None
...
class Cache:
    ...
    def insert(self, content, evictionPolicy):
        ...
        return self.hierarchy[hashVal].put(content, evictionPolicy)


    def __getitem__(self, content):
        # YOUR CODE STARTS HERE
        hashVal = hash(content)
        lst = self.hierarchy[hashVal]
        if content.cid in lst:
...
def run_tests():
    ...
    # doctest.run_docstring_examples(CacheList, globals(), name='HW3',verbose=False)   
